...
def check_for_win(board):
    ...
    for i in range(len(board)):
        if board[0][i]==board[1][i] and boarod[1][i] ==boardr[2][i] and board[2][i]!="":
            return True
    if board[0][0]==board[1][1] and boarod[1][1] ==boardr[2][2] and board[2][2]!="":
            return True
    if board[0][2]==board[1][1] and boarod[1][1] ==boardr[2][0] and board[2][0]!="":
            return True
    return False
...
def get_markers():
    ...
    if marker1=="X":
        ...
        return get_markers()
def get_cordinates():
    x,y=list(map(int,input("Enter the cordinaTES:").split()))
    if x in [0,1,2] and y in [0,1,2]:
        return[x,y]
    else:
        print("Invalid Input")
...
def check_for_win(board):
    ...
    for i in range(len(board)):
        if board[0][i] ==board[1][i] and board[1][i]==board[2][i] and board[2][i]!="":
            return True
        if board[0][0] == board [1][1] and board[1][1] and board [2][2] and board [2][2]!="":
            return True
        if board[0][2] ==board[1][1] and board[1][1] and board[2][0] and board[2][0]!="":
            return True
    return False
def update_board(board,chance,marker,x,y):
    if chance==True:
